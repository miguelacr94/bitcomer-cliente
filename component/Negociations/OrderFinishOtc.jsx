...
const OrderFinishOtc = () => {


    const { formOtc } = useContext(Context);
    const { quantityCalcule } = useContext(Context);
    const { countrySelect } = useContext(Context);
    const { quantityCalculeOtc } = useContext(Context);
        ...
        <div className=" w-full  lg:ml-4 lg:mr-4 border border-menu rounded-3xl  text-start pt-4 px-2 lg:px-8 lg:mt-0 mt-8 pb-12 mb-12 overflow-x-hidden">
            ...
            <div className="flex mt-4">
                ...
                <p className="w-3/6 text-end  text-sm text-grey-light -ml-4 flex justify-end space-x-2 items-center">
                    <img
                        src={formOtc?.sale?.image}
                        className="w-4 h-4"
                    />
                    <spam> {formOtc ? formOtc?.sale?.name : ''}</spam></p>
            </div>
            <div className="flex mt-4 items-center">
                <p className="w-4/6 text-start text-sm  text-menu font-semibold text-order">Cantidad en criptomoneda:</p>
                <p className="w-2/6 text-end  text-sm text-grey-light  -ml-4 flex justify-end items-center"><span>{formOtc?.quantity ? formOtc?.quantity : 0}</span> <span>{Icons.coin}</span> </p>
            </div>
            <div className="flex mt-4">
                ...
                ${quantityCalculeOtc?.commission > 0 ? 'text-green-400' : 'text-red-400'}
                
                `}>{quantityCalculeOtc ? quantityCalculeOtc?.commission * 100 : 0}%</p>
            </div>
            <div className="flex mt-4">
                <p className="w-2/6 text-start text-sm  text-menu font-semibold text-order">Monto a recibir:</p>
                <p className="w-4/6 text-end  font-bold text-xl text-totalOrder">{quantityCalculeOtc ? currencyFormat(quantityCalculeOtc?.value) : 0} USD</p>
            </div>
